      ...
      <main className="mx-auto max-w-4xl px-5 py-12 sm:px-8">
        <div className="flex items-end justify-between gap-3">
          <div>
            ...
            <h1 className="mt-1.5 font-display text-5xl text-graphite sm:text-6xl">Scoreboard</h1>
          </div>
          <LivePill live={live} />
        </div>
        ...
        <motion.div initial="hidden" animate="show" variants={stagger} className="mt-7 grid grid-cols-3 gap-3">
          {stats.map((s) => (
            <motion.div key={s.label} variants={fadeUp} className="rounded-3xl bg-white px-4 py-4 text-center shadow-card ring-1 ring-black/5">
              <div className="font-display text-3xl text-graphite tabular">{s.value}</div>
